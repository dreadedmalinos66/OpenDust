...
#define SD_CS_PIN 5
WebServer server(80); //local hosted web site port
...
void handleFileRead() {
    ...
    // Default to /index.html if accessing the root URL "/"
    if (path.endsWith("/")) {
        path += "index.html";
    }
    ...
    if (SD.exists(path)) {
        ...
        // Stream the file directly to the web browser
...
void web_server_init() {
    ...
    // Direct all incoming HTTP requests to handleFileRead
